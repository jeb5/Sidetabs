...
async function updateThemeStyle(
  ...
  let darkMode: boolean;
  {
    const tabTextColorString =
      newTheme.colors[
        themeStyleColorMappings["--theme-tab-text-color"].find(
          (key) => newTheme.colors[key]
        ) || "tab_text"
      ];
    darkMode = tabTextColorString
      ? Color(tabTextColorString).isLight()
      : windowInDarkMode; //If tab text is light, theme must be dark. If tab text is dark, theme must be light. If tab text is undefined, use window dark mode.
  }
  ...
  const images = includeAdditionalImages
    ? newTheme.images.additional_backgrounds?.map((image, index) => {
        return {
          image,
          alignment:
            newTheme.properties.additional_backgrounds_alignment?.[index] ||
            "center",
          tiling:
            newTheme.properties.additional_backgrounds_tiling?.[index] ||
            "no-repeat",
        };
      }) || []
    : [];
  ...
  let newSidebarTheme: SidebarTheme | null = detectExceptionThemes(
    theme,
    windowInDarkMode
  );
  if (newSidebarTheme) {
    ...
    darkMode = (
      newSidebarTheme.colors["--theme-tab-text-color"] as Color
    ).isLight();
  } else {
    newSidebarTheme = {
      colors: Object.fromEntries(
        Object.entries(themeStyleColorMappings).map(([cssVar, colorKeys]) => {
          ...
          const color = availableKey
            ? Color(newTheme.colors[availableKey])
            : defaultScheme.colors[cssVar];
          return [cssVar, color];
        })
      ),
      images,
  ...
  // ---- Extra colors ---
  const tabTextColor = newSidebarTheme.colors[
    "--theme-tab-text-color"
  ] as Color;
...
function getThemeFingerprint(theme: Theme) {
  ...
  const cleanedJSON = JSON.stringify(theme).replace(
    /moz-extension:\/\/[\w-]*\//gm,
    "EXTENSION:"
  );
...
function detectExceptionThemes(
  theme: Theme,
  windowInDarkMode: boolean
): SidebarTheme | null {
  // --- Exception themes ---
  ...
  switch (getThemeFingerprint(theme)) {
    case "267e0e07e5cd597938953b5a4d7e6717": //Alpenglow
      ...
      return windowInDarkMode
        ? DEFAULT_THEMES.ALPENGLOW_DARK
        : DEFAULT_THEMES.ALPENGLOW_LIGHT;
...
export async function stylesFromSidebarTheme(theme: SidebarTheme) {
  ...
  const cssColorVars = Object.entries(theme.colors)
    .map(([cssVar, color]) => {
      const strValue =
        color instanceof Color ? color.rgb().array().join(", ") : color;
    ...
    ? "--theme-bg-image: " +
      theme.images
        .map(({ image, alignment, tiling }, index) => {
          ...
          const imgIsFull =
            (imageSizes[index].width > 500 && imageSizes[index].height > 50) ||
            tiling !== "no-repeat"; //Is the image intended to fill the whole frame?
          if (imgIsFull) {
...
const useTheme = (extensionOptions: OptionForm) => {
  ...
  useEffect(() => {
    if (extensionOptions["theme/mode"] === "dark")
      return setTheme(DEFAULT_THEMES.DEFAULT_DARK_SIDEBAR_THEME);
    if (extensionOptions["theme/mode"] === "light")
      return setTheme(DEFAULT_THEMES.DEFAULT_LIGHT_SIDEBAR_THEME);
    ...
    const setNewTheme = async (newTheme: Theme) => {
      if (newTheme)
        setTheme(
          await updateThemeStyle(newTheme, showImages, showAdditionalImages)
        );
    };

    const themeListener = async ({
      theme,
    }: {
      theme: browser.Theme.ThemeUpdateInfoThemeType;
    }) => {
      const newTheme = theme as Theme;
