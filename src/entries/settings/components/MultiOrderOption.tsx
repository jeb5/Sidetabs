...
export const MultiOrderOption = <T,>(props: {
  ...
  return (
		<Controller
			...
			name={props.name}
			render={({ field: { onChange, value } }: { field: { onChange: (newState: string[]) => void; value: string[] } }) => (
					...
					<div className="multi-order-current multi-order-options">
						...
						<DragAndDrop
							items={value.map((value) => ({
								name: value,
								label: props.availableOptionLabels[value],
								id: value,
							}))}
							onDragEnd={(reorderInfo?: { fromIndex: number; toIndex: number }) => {
								if (reorderInfo) onChange(arrWithReposition(value, reorderInfo.fromIndex, reorderInfo.toIndex));
							}}
