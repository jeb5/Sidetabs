...
import { WindowInfoContext } from "../sidebar/Root";
...
async function updateThemeStyle(theme: Theme, includePrimaryImage: boolean = true, includeAdditionalImages: boolean = true) {
	...
	const extraColors = {
		"--theme-tab-hover-color": tabTextColor ? Color("transparent").mix(tabTextColor, 0.11) : Color("rgba(255,255,255,0.11)"),
...
const useTheme = (extensionOptions: OptionForm) => {
	...
	const WIN_ID = windowInfo?.windowId!;
...
export function ThemeContextProvider(props: { children: React.ReactNode }) {
	...
	const themeProviderValue =
		theme != null ? { theme, dark: themeIsDark } : { theme: DEFAULT_THEMES.DEFAULT_DARK_SIDEBAR_THEME, dark: true };

	return <ThemeContext.Provider value={themeProviderValue}>{props.children}</ThemeContext.Provider>;
}
