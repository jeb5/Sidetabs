...
function tabMenuItems(tab: Tab, options: OptionForm): MenuStructure[] {
  const ctxMenuItems: {
    ...
    reopen: {
      ...
      children: [
        ...
        ...containers.map((container) => ({
          ...
          onclick: () =>
            TabMethods.reopenWithCookieStoreId(tab, container.cookieStoreId),
        })),
      ],
    },
    unload: {
    ...
    clearData: {
      ...
      icons: ctxIcons.clearData,
    },
    close: {
      title: "Close Tab",
      onclick: () => TabMethods.close(tab),
      icons: ctxIcons.closeTab,
    },
  };

  return [
    ...options["ctxMenu/menuItems"].map((item) => ctxMenuItems[item]),
    ...(options["ctxMenu/showCloseOption"]
      ? [{ type: "separator" } as MenuStructure, ctxMenuItems.close]
      : []),
  ];
}

//Returns the MenuStructure array for items unrelated to a specific tab
function defaultMenuItems(tab?: Tab): MenuStructure[] {
  return [
    {
      title: "New Tab",
...
function createContext(
  ...
  for (const childContextObj of children || [])
    createContext(childContextObj, showIcons, id);
}
