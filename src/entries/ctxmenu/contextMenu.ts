...
import { Tab, TabManager, TabManagerContext } from "../sidebar/TabManager";
...
function tabMenuItems(iconsColor: "black" | "white", tab: Tab, options: OptionForm, tabManager: TabManager): MenuStructure[] {
	...
	const ctxMenuItems: {
		...
		addToGroup: {
			...
			children: [
				{
					title: "New Group",
					onclick: () => tabManager.addNewGroup([tab]),
				}
			]
		},
		reload: {
	...
	if (tab) {
		return function showContextMenu(event: React.MouseEvent) {
			...
			const menuItems: MenuStructure[] = [
				...
				{ type: "separator" },
				...tabMenuItems(iconsColor, tab, options, tabManager),
			];
